...
def fileAdvanceOnlyCheck(move):
    return fileAdvanceCheck(EvaluationMove(move.fromSquare, move.toSquare)) and not (
        rankAdvanceCheck(EvaluationMove(move.fromSquare, move.toSquare)))


def rankAdvanceOnlyCheck(move):
    return not (fileAdvanceCheck(EvaluationMove(move.fromSquare, move.toSquare))) and rankAdvanceCheck(
        EvaluationMove(move.fromSquare, move.toSquare))


def fileOrRankAdvanceExclusiveCheck(move):
    return not (fileAdvanceOnlyCheck(EvaluationMove(move.fromSquare, move.toSquare)) == rankAdvanceOnlyCheck(
        EvaluationMove(move.fromSquare, move.toSquare)))


def fileOrRankAdvanceBothCheck(move):
    return fileAdvanceCheck(EvaluationMove(move.fromSquare, move.toSquare)) and rankAdvanceCheck(
        EvaluationMove(move.fromSquare, move.toSquare))
